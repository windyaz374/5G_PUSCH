...
% uplink waveform. The following channels and signals can be generated:
%
...
% bandwidth parts (BWP). Multiple instances of PUSCH, PUCCH and SRS can be
% generated over the different BWPs. The example allows to configure PUCCH,
% PUSCH and SRS for a specific UE categorized by RNTI and transmits only
...
pucch = [];
pucch(1).Enable = 0;                        % Disable PUCCH sequence 
...
% enabled. This value is ignored, when PortSet field is specified
...
% number of DM-RS ports configured
...
pusch(1).InterSlotFreqHopping = 'disabled';  % Inter-slot frequency hopping ('enabled','disabled')
...
pusch(1).PortSet = [0];              % DM-RS antenna ports to use for the layers, when field is specified
...
pusch(1).DMRSAdditionalPosition = 1;   % Additional DM-RS symbol positions (max range 0...3)
pusch(1).NIDNSCID = 0;                 % Scrambling identity for CP-OFDM (0...65535). Use empty ([]) to use physical layer cell identity
...
pusch(1).GroupHopping = 'disable';      % {'enable','disable','neither'}. This parameter is used only when transform precoding is enabled
...
% This section specifies the parameters for the set of SRS instances in the
% waveform. Each element in the structure array defines an SRS sequence
% instance. This example defines two SRS sequence instances that are
% disabled. The following parameters can be set:
%
...
% * Bandwidth and frequency hopping configuration. The occupied bandwidth
% depends on the parameters |CSRS|, |BSRS|, and |BHop|. Set |BHop < BSRS| to
...
% * Cyclic shift rotating the low-PAPR base sequence. The maximum number of
% cyclic shifts, 8 or 12, depends on the transmission comb number, 2 or 4.
% For 4 SRS antenna ports, the subcarrier set allocated to the SRS in the
% first and third antenna ports depends on the cyclic shift.
...
% * Scrambling identity. It initializes the pseudo-random binary sequence
% when group or sequence hopping are enabled.
...
srs(1).CSRS = 10;                   % Bandwidth configuration C_SRS (0...63). It controls the allocated bandwidth to the SRS 
...
% generated:
%
% * Resource grid showing the location of the components (PUCCH, PUSCH and
% SRS) in each BWP. This does not plot the power of the signals, just their
...
% the PUCCH, PUSCH and SRS instances
%%
